%Sensitivity on k
function [f_ss,T,Q_used,y_mass,Vapor_flow_rate,MDEA_recovery] = ss_sensitivity(x,P_tot,waste,V,k_free)
global p     
f_ss=zeros(length(p.PM),1); %EACH COMPONENTS PLUS TEMPERATURE
...
c_aceticacid = c(6);
c_h2so4 = c(7);
r_free_hcl=k_free*c_naoh*c_hcl;    %Declaring GEN/CONS term
r_free_formicacid = k_free*c_naoh*c_formicacid;
r_free_aceticacid = k_free*c_naoh*c_aceticacid;
r_free_h2so4 = k_free*c_naoh*c_h2so4;
...
for i = 1:length(mf)
     f_ss(i) = (p.m_in(i)-Vapor_flow_rate*y_mass(i)-waste_mass*wf(i)+ r_free_hcl*V*p.vi_free_hcl(i)*p.PM(i)+r_free_formicacid*V*p.vi_free_formicacid(i)*p.PM(i)+r_free_aceticacid*V*p.vi_free_aceticacid(i)*p.PM(i)+r_free_h2so4*V*p.vi_free_h2so4(i)*p.PM(i))/Mtot;
end
...
%RECOVERY evaluation
MDEA_recovery = y_mass(1)*Vapor_flow_rate/(p.m_in(1));
end                                                                                                                                             
